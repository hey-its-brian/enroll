...
When(/^.+ completes? the matched employee form for (.*)$/) do |named_person|
  ...
  actual_phone = find_field("person[phones_attributes][0][full_phone_number]").value
  expect(page).to have_field("HOME PHONE", with: actual_phone) if person[:home_phone].present?
