...
# @class MigrateExistingTransferredApplicationToPopulateOriginReason
# This migration updates existing transferred in Financial Assistance applications
...
#   RAILS_ENV=production bundle exec rails db:migrate:up VERSION="20250502105840"
#
# @see Mongoid::Migration
class MigrateExistingTransferredApplicationToPopulateOriginReason < Mongoid::Migration

  def self.up
    if EnrollRegistry.feature_enabled?(:qhp_application)
      # Fetch applications with a non-nil `transfer_id`
      puts "Fetching applications with non-nil transfer_id..."
      applications = FinancialAssistance::Application.where(:transfer_id.ne => nil)
  ...
  def self.generate_csv(array_collection)
    ...
    file_name = "#{Rails.root}/atp_inbound_migrated_applications_#{Date.today.strftime('%Y_%m_%d')}.csv"
