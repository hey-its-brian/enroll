...
primary_person_hbx_ids.each do |primary_person_hbx_id|
  ...
  if family.present? && app.present?
    ...
    puts "no primary family for the given person or valid QHP is present for person HBX ID: #{primary_person_hbx_id}"
  end
...
puts "oeq_catch_up_notice_triggers end_time: #{end_time}, total_time_taken_in_minutes: #{((end_time - start_time) * 24 * 60).to_f.ceil}"
