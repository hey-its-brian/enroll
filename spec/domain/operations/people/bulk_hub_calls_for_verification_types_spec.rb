...
RSpec.describe ::Operations::People::BulkHubCallsForVerificationTypes, dbclean: :after_each do
  ...
  context 'when there are multiple person records' do
    ...
    it 'should return a success message' do
      expect(@result.success).to eq([[person.hbx_id, person.verification_types.ssn_type.first.type_name, "Request was sent to FedHub."], [person1.hbx_id, person1.verification_types.ssn_type.first.type_name, "not eligible for hub call"]])
    end
  end
end