...
RSpec.describe Operations::Families::Verifications::Summary::HouseholdQuery, dbclean: :after_each do
  ...
  context 'when family determination does not exist' do
    it 'returns an error when family determination is not found' do
  ...
  context 'valid params' do
    ...
    it 'returns member and evidences for a valid family and person_id' do
      ...
      expect(result.success[:action_items].map(&:evidence_item_key)).to include(:non_esi_evidence)
      expect(result.success[:subjects]).to be_present
    end
  end
end
