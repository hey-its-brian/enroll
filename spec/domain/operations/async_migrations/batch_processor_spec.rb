...
RSpec.describe Operations::AsyncMigrations::BatchProcessor, dbclean: :after_each do
    ...
    describe '#process_migration_batch' do
      ...
      context 'when batch is processed successfully' do
        it 'returns a success monad containing a success message' do
          ...
          expect(result).to be_a(Dry::Monads::Result::Success)
          expect(result.success).to eq("process_migration_for_mongo_objects: Published batch of events for migration using params: #{params}")
        end
  ...
  describe '#for array' do
    ...
    let(:params) do
      {
        ...
        batch_size: batch_size,
        records: (1..50).to_a,
        additional_params: { assistance_year: TimeKeeper.date_of_record.year, data_type: 'Array'}
      }
    end
    let(:mocked_event) { double(publish: true) }
    ...
    before do
      allow(EventSource::Event).to receive(:new).and_return(mocked_event)
      allow(Operations::AsyncMigrations::Handlers::FAApplication::FetchApplicationsWithoutV3Evidences.new).to receive(:call).and_return((1..100).to_a)
    end

    context 'when data_type is Array' do
      it 'processes the migration batch for array' do
        result = subject.call(params)
        expect(result).to be_a(Dry::Monads::Result::Success)
        expect(result.success).to eq("process_migration_batch_for_array: Published batch of events for migration using params: #{params}")
      end
    end
  end
end
