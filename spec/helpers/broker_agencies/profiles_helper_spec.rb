...
RSpec.describe BrokerAgencies::ProfilesHelper, dbclean: :after_each, :type => :helper do
  ...
  let!(:organization_assister)                  { FactoryBot.create(:benefit_sponsors_organizations_general_organization, :with_assister_agency_profile, site: site) }
  let!(:broker_agency_profile1) { organization.broker_agency_profile }
  let!(:assister_agency_profile1) { organization_assister.assister_agency_profile }
  ...
  let!(:assister_role) { FactoryBot.create(:assister_role, aasm_state: 'active', benefit_sponsors_assister_agency_profile_id: assister_agency_profile1.id, person: person) }
  let!(:assister_role1) { FactoryBot.create(:assister_role, aasm_state: 'active', benefit_sponsors_assister_agency_profile_id: '123', person: person3) }
  ...
  describe 'disable_edit_assister_agency?' do
    it 'should return false if current user has assister role' do
      allow(person).to receive(:assister_role).and_return true
      expect(helper.disable_edit_assister_agency?(user)).to be_falsey
    end

    it 'should return true if current user does not have assister role' do
      allow(person).to receive(:assister_role).and_return false
      expect(helper.disable_edit_assister_agency?(user)).to eq true
    end
  ...
  describe 'can_show_destroy_for_assisters?' do
    context "total assister staff count" do
      it "should return false if single staff member remains" do
        expect(helper.can_show_destroy_for_assisters?(person, 1, assister_agency_profile1)).to be_falsey
      end
    end

    context "with assister role" do
      before do
        assister_agency_profile1.update_attributes!(primary_assister_role: assister_role)
      end

      it 'should return false if staff has primary assister role' do
        expect(helper.can_show_destroy_for_assisters?(person, 2, assister_agency_profile1)).to be_falsey
      end

      it 'should return true if staff DOES NOT HAVE assister role' do
        expect(helper.can_show_destroy_for_assisters?(person2, 2, assister_agency_profile1)).to be_truthy
      end

      it 'should return true if staff has assister role, but it is not primary' do
        expect(helper.can_show_destroy_for_assisters?(person3, 2, assister_agency_profile1)).to be_truthy
      end
    end
  end
