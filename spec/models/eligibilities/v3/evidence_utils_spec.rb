...
RSpec.describe Eligibilities::V3::EvidenceUtils do
  ...
  describe 'state transition criteria' do
    ...
    describe "#latest_rejected_verification_history" do
      let(:person) { FactoryBot.create(:person, :with_consumer_role, :with_active_consumer_role) }
      let(:family) { FactoryBot.create(:family, :with_primary_family_member, person: person) }
      let(:primary_applicant) { family.primary_applicant }
      let(:faa_application) do
      ...
      let(:ivl_eligibility) { FactoryBot.create(:individual_market_eligibility, eligible: applicant) }
      let(:citizenship_evidence) { FactoryBot.create(:citizenship_evidence, :rejected, eligibility: ivl_eligibility) }
      let(:rejected_vh1) { FactoryBot.build(:v3_verification_history, evidence: citizenship_evidence, action: 'return_for_deficiency', update_reason: 'test_reason1', updated_by: 'test_user1') }
      let(:rejected_vh2) { FactoryBot.build(:v3_verification_history, evidence: citizenship_evidence, action: 'return_for_deficiency', update_reason: 'test_reason2', updated_by: 'test_user2') }
      let(:non_rejected_vh) { FactoryBot.build(:v3_verification_history, evidence: citizenship_evidence, action: 'other_action', update_reason: 'other_reason', updated_by: 'other_user') }

      context "when rejected verification histories are built in order" do
        let(:create_embed_docs) do
          [citizenship_evidence].each do |evidence|
            FactoryBot.build(:v3_state_history, status_trackable: evidence, created_at: 2.days.ago)
            evidence.verification_histories << rejected_vh1
            evidence.verification_histories << non_rejected_vh
            evidence.verification_histories << rejected_vh2
            evidence.documents.build(title: 'document.pdf', creator: 'mehl', subject: 'document.pdf', publisher: 'mehl', type: 'text', identifier: 'identifier', source: 'enroll_system', language: 'en')
          end
        ...
        it "should fetch the last rejected verification history" do
          create_embed_docs
          expect(citizenship_evidence.latest_rejected_verification_history).to eq(rejected_vh2)
        end
      end

      context "when rejected verification histories are built in different order" do
        let(:create_embed_docs) do
          [citizenship_evidence].each do |evidence|
            FactoryBot.build(:v3_state_history, status_trackable: evidence, created_at: 2.days.ago)
            evidence.verification_histories << rejected_vh2
        ...
        it "should fetch the last rejected verification history that was saved" do
          create_embed_docs
          expect(citizenship_evidence.latest_rejected_verification_history).to eq(rejected_vh1)
        end
      end

      context "when there are no rejected verification histories" do
        let(:create_embed_docs) do
          [citizenship_evidence].each do |evidence|
            FactoryBot.build(:v3_state_history, status_trackable: evidence, created_at: 2.days.ago)
            evidence.verification_histories << non_rejected_vh
        ...
        it "should return nil" do
          create_embed_docs
          expect(citizenship_evidence.latest_rejected_verification_history).to be_nil
        end
      end

      context "when result is memoized" do
        let(:create_embed_docs) do
          [citizenship_evidence].each do |evidence|
            evidence.verification_histories << rejected_vh1
            evidence.verification_histories << rejected_vh2
        ...
        it "returns the memoized result on subsequent calls" do
          create_embed_docs
          result = citizenship_evidence.latest_rejected_verification_history
          expect(citizenship_evidence.latest_rejected_verification_history).to eq(result)
          expect(citizenship_evidence.latest_rejected_verification_history).to eq(rejected_vh2)
        end
      end
    end
