...
RSpec.describe Eligibilities::V3::Evidences::ImmigrationEvidence, type: :model do
  ...
  let(:evidence)        { FactoryBot.create(:immigration_evidence, eligibility: ivl_eligibility) }
  ...
  describe 'verification workflow' do
    let(:evidence) { FactoryBot.create(:immigration_evidence, :pending, eligibility: ivl_eligibility) }

    it 'starts in pending state' do
      expect(evidence.current_state).to eq(:pending)
    end

    context 'when immigration documents are uploaded' do
      before do
    ...
    context 'when immigration is verified' do
      before do
