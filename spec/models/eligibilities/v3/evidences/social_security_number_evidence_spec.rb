...
RSpec.describe Eligibilities::V3::Evidences::ImmigrationEvidence, type: :model do
  ...
  describe 'verification workflow' do
    let(:evidence) { FactoryBot.create(:social_security_number_evidence, :pending, eligibility: ivl_eligibility) }

    it 'starts in pending state' do
      expect(evidence.current_state).to eq(:pending)
    end

    context 'when social_security_number documents are uploaded' do
      before do
    ...
    context 'when social_security_number is verified' do
      before do
