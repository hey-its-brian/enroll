...
RSpec.describe Eligibilities::V3::Evidences::CitizenshipEvidence, type: :model do
  ...
  describe 'verification workflow' do
    let(:evidence) { FactoryBot.create(:citizenship_evidence, :pending, eligibility: ivl_eligibility) }

    it 'starts in pending state' do
      expect(evidence.current_state).to eq(:pending)
    end

    context 'when citizenship documents are uploaded' do
      before do
    ...
    context 'when citizenship is verified' do
      before do
