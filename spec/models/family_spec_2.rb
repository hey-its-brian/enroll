...
RSpec.describe Family, dbclean: :around_each do
  ...
  describe '#application_for_year' do
   ...
   # TODO: Update this test to use the way of identifying the correct applications
  describe '#show_previous_year_faa_verifications' do
    let(:family) { FactoryBot.create(:family, :with_primary_family_member) }
    ...
    context 'when the family has a previous year faa application needing verifications' do
      let(:previous_year) { TimeKeeper.date_of_record.year - 1 }
      let(:previous_year_faa_application_needing_verifications) { FactoryBot.create(:financial_assistance_application, family_id: family.id, assistance_year: previous_year, aasm_state: 'determined') }

      context 'when the latest application is a qhp application' do
        before do
          allow(family).to receive(:latest_application_type).and_return('qhp')
          previous_year_faa_application_needing_verifications
        end

        it 'returns true' do
          expect(family.show_previous_year_faa_verifications).to be_truthy
        end
      end

      context 'when the latest application is a faa application' do
        before do
          allow(family).to receive(:latest_application_type).and_return('faa')
        end

        it 'returns false' do
          expect(family.show_previous_year_faa_verifications).to be_falsey
        end
      end
    end

    context 'when the family does not have a previous year faa application needing verifications' do
      before do
  ...
  # TODO: Update this test to use the way of identifying the correct applications
  describe '#previous_year_faa_application_needing_verifications' do
    let(:family) { FactoryBot.create(:family, :with_primary_family_member) }
    ...
    context 'when the family has a previous year faa application needing verifications' do
      ...
      it 'returns the previous year faa application needing verifications' do
        expect(family.previous_year_faa_application_needing_verifications).to eq(previous_year_faa_application_needing_verifications)
      end
    end

    context 'when the family has a older year faa application needing verifications' do
      let(:older_year) { TimeKeeper.date_of_record.year - 2 }
      let(:faa_application_needing_verifications) { FactoryBot.create(:financial_assistance_application, family_id: family.id, assistance_year: older_year, aasm_state: 'determined') }

      it "returns nil" do
        expect(family.previous_year_faa_application_needing_verifications).to be_nil
      end
    end
  end
end
