...
RSpec.describe Locations::Phone, type: :model do
  ...
  let(:phone) { FactoryBot.create(:location_phone, phoneable: applicant) }

  subject { Locations::Phone }

  describe 'associations' do
    it 'is embedded in the phoneable' do
      expect(phone).to be_a(Locations::Phone)
      expect(phone.phoneable).to eq(applicant)
    end
  end
