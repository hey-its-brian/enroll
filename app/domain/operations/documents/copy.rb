...
module Operations
  module Documents
    # This class copies a document from cartafact to a paper notices bucket
    class Copy
      ...
      def call(opts)
        ...
        if opts[:resource].present? && opts[:document].present?
        ...
        response = yield copy(document, header)
      ...
      def copy(document, header)
        if Rails.env.production?
          response = HTTParty.get(fetch_url + "/#{document.doc_identifier}/copy", :headers => header)

          response.code == 200 ? Success(response) : Failure({:message => l10n("hbx_profiles.copy_tax_form_document.failure_message")})
        else
          Failure({:message => l10n("hbx_profiles.copy_tax_form_document.failure_message")})
        end
      end
    end
  end
