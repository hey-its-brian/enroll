  ...
  class FamilyPvcDeterminationSubscriber
    ...
    subscribe(:on_enroll_iap_family_pvc_determination_events) do |delivery_info, _metadata, response|
      ...
      result = if qhp_application_feature_enabled?
                 ::FinancialAssistance::Operations::Applications::Pvc::NonEsiEvidence::RequestDetermination.new.call({application_hbx_id: payload[:application_hbx_id]})
               else
                 ::FinancialAssistance::Operations::Applications::Pvc::CreatePvcRequest.new.call({family_hbx_id: payload[:family_hbx_id],
