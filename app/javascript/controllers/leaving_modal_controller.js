...
export default class extends Controller {
  ...
  modalWhenLeavingFormUnfinished() {
    const headerLinks = Array.from(document.querySelectorAll('header a, .progress-nav-container a')).filter(link => !link.closest('.progress-nav')).filter(link => !link.dataset.target).filter(link => link.href.length > 1)
