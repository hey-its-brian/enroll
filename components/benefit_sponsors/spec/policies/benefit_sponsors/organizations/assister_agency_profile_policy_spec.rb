...
module BenefitSponsors
  RSpec.describe Organizations::AssisterAgencyProfilePolicy  do
    let(:assister_agency_profile_id) { "BROKER AGENCY PROFILE ID" }
    let(:user) { instance_double(User, person: person) }
    let(:assister_agency_profile) { instance_double(::BenefitSponsors::Organizations::AssisterAgencyProfile, :id => assister_agency_profile_id) }
    let(:fake_assister_agency_profile) { instance_double(::BenefitSponsors::Organizations::AssisterAgencyProfile, :id => "34509823749514314") }
    let(:policy) { ::BenefitSponsors::Organizations::AssisterAgencyProfilePolicy.new(user, assister_agency_profile) }
    ...
    describe "given a user with no person" do
      ...
      it_behaves_like "is not permitted", :access_to_assister_agency_profile?
    ...
    describe "given an admin" do
      ...
      context "with higher-level access" do
        ...
        it_behaves_like "is permitted", :access_to_assister_agency_profile?
    ...
    describe "given a assister agency staff role for that profile" do
      let(:person) do
        instance_double(
          Person,
          :hbx_staff_role => nil,
          :assister_role => nil,
          :assister_agency_staff_roles => [assister_agency_staff_role]
        )
      end

      let(:assister_agency_staff_role) do
        instance_double(
          AssisterAgencyStaffRole,
          :assister_agency_profile_id => nil,
    ...
    # it should be noted that the :index? and :staff_index? methods are not specific to assister agency profile
    # they are omitted from the 'for a different profile' contexts because they would be redundant
    describe "given a assister agency staff role for a different profile" do
      let(:person) do
        instance_double(
          Person,
          :hbx_staff_role => nil,
      ...
      let(:assister_agency_staff_role) do
        instance_double(
          AssisterAgencyStaffRole,
          :assister_agency_profile_id => nil,
          :benefit_sponsors_assister_agency_profile_id => fake_assister_agency_profile.id,
          :active? => true
        )
      end
    ...
    describe "given a assister role for that profile" do
      let(:person) do
        instance_double(
          Person,
          :hbx_staff_role => nil,
    ...
    # it should be noted that the :index? and :staff_index? methods are not specific to assister agency profile
    # they are omitted from the 'for a different profile' contexts because they would be redundant
    describe "given a assister role for a different profile" do
      let(:person) do
        instance_double(
          Person,
          :hbx_staff_role => nil,
          :assister_role => fake_assister_role,
          :assister_agency_staff_roles => []
        )
      end
