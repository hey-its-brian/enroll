...
module BenefitSponsors
  RSpec.describe Organizations::OrganizationForms::StaffRoleFormPolicy, dbclean: :after_each  do
    ...
    let(:assister_policy)                          { BenefitSponsors::Organizations::OrganizationForms::StaffRoleFormPolicy.new(user, assister_staff_role_form) }
    let(:assister_staff_role_form) do
    ...
    describe 'for a AssisterAgencyStaffRoleForm' do
      context "an hbx_admin " do
        ...
        context 'with a super_admin role' do
          shared_examples_for "is permitted" do |policy_type|
            it "to #{policy_type} a AssisterStaffAgencyRole form" do
              expect(assister_policy.send(policy_type)).to be_truthy
            end
          end
        ...
        # NOTE: permissions for assister agency are frequently subject to change, specs built out for both kinds of hbx_staff_role
        context 'with a read-only role' do
          before do
            user.person.hbx_staff_role.permission_id = read_only_permission.id
            user.person.hbx_staff_role.save!
      ...
      context 'as a member of a assister agency' do
        context 'a assister' do
          ...
          let!(:assister_agency_staff_role)      { FactoryBot.create(:assister_agency_staff_role, benefit_sponsors_assister_agency_profile_id: assister_agency_profile.id, person: person) }
        ...
        context 'an agent' do
          let!(:assister_agency_staff_role) do
            FactoryBot.create(:assister_agency_staff_role, benefit_sponsors_assister_agency_profile_id: assister_agency_profile.id, person: person, aasm_state: 'active')
          end
      ...
      context 'as a member of a different agency' do
        ...
        let!(:profile_2)                     { second_organization.assister_agency_profile }

        context 'a assister' do
          let!(:assister_role)                   { FactoryBot.create(:assister_role, aasm_state: 'active', benefit_sponsors_assister_agency_profile_id: profile_2.id, person: person) }
          let!(:assister_agency_staff_role)      { FactoryBot.create(:assister_agency_staff_role, benefit_sponsors_assister_agency_profile_id: profile_2.id, person: person) }

          shared_examples_for "is not permitted" do |policy_type|
            it "to #{policy_type} a AssisterStaffAgencyRole form" do
              expect(assister_policy.send(policy_type)).to be_falsey
            end
          end
        ...
        context 'an agent' do
          let!(:assister_agency_staff_role) do
            FactoryBot.create(:assister_agency_staff_role, benefit_sponsors_assister_agency_profile_id: profile_2.id, person: person, aasm_state: 'active')
          end
