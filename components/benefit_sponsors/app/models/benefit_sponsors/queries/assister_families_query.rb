...
module BenefitSponsors
  module Queries
    # query class for assister search in families
    class AssisterFamiliesQuery

      def initialize(s_string, assister_agency_id, market_kind)
        @use_search = !s_string.blank?
        @search_string = s_string
        @assister_agency_profile_id = assister_agency_id
        @market_kind = market_kind
      end

      def build_base_scope
        ivl_assister_agency_criteria = { assister_agency_accounts: {:$elemMatch => {benefit_sponsors_assister_agency_profile_id: @assister_agency_profile_id, is_active: true}} }
        shop_assister_agency_criteria = { "family_members.person_id" => {"$in" => employee_person_ids }}
        if @market_kind == :shop
          shop_assister_agency_criteria
        else
          { "$or" => [
            ivl_assister_agency_criteria,
            shop_assister_agency_criteria
          ]}
        end
      end
      ...
      def employee_person_ids
        benefit_sponsorships = BenefitSponsors::BenefitSponsorships::BenefitSponsorship.where(
          :assister_agency_accounts => {"$elemMatch" => {:benefit_sponsors_assister_agency_profile_id => @assister_agency_profile_id, is_active: true}}
        )
