...
RSpec.describe ::FinancialAssistance::Operations::Transfers::MedicaidGateway::V2::AccountTransferIn, dbclean: :after_each do
  ...
  context 'success' do
    ...
    context 'with valid payload' do
      ...
      context 'no ssn' do
        context 'person' do
          it 'should should populate person no_ssn field using transformed cv3 person demographics' do
            person_demographics = @transformed["family"]["family_members"].first["person"]["person_demographics"]
            no_ssn_string = ::FinancialAssistance::Operations::Transfers::MedicaidGateway::V2::BuildFamilyAndCreateMember.new.transform_no_ssn(person_demographics["ssn"])
            person = Person.first
      ...
      context 'with ethnicity' do
        it 'should have an ethnicity' do
          person = Person.all.first
          expect(person.ethnicity).to eq ["White"]
        end
      end
      ...
      context 'relationships' do
        ...
        it 'should create the only the primary family member relationships' do
          expect(@family_member_rels).to eq ["self"]
        end
  ...
  context 'AI/AN status' do
    ...
    context "when PersonAmericanIndianOrAlaskaNativeIndicator is set to true but no tribe name and tribe state" do
      ...
      before do
        ...
        @applicant = FinancialAssistance::Application.first.applicants.where(first_name: "Junior").first
      end

      it "should set tribe_codes to empty array on applicant" do
        expect(@applicant.tribe_codes).to eq []
      end

      it "should set applicant indian_tribe_member status to false" do
        expect(@applicant.indian_tribe_member).to eql(false)
      end
    end
    ...
    context "when person records exits and the payload is missing PersonAmericanIndianOrAlaskaNativeIndicator element" do
      ...
      let!(:consumer_role) { FactoryBot.create(:consumer_role, lawful_presence_determination: lawful_presence_determination, person: person) }
      let!(:lawful_presence_determination) { FactoryBot.build(:lawful_presence_determination, citizen_status: 'us_citizen') }
    ...
    context "When payload is missing PersonAmericanIndianOrAlaskaNativeIndicator element" do
      let(:document) do
        document = Nokogiri::XML(xml)
        document.xpath('//ns4:PersonAmericanIndianOrAlaskaNativeIndicator', 'ns4' => "http://hix.cms.gov/0.1/hix-core")[1].remove
        document
      ...
      it "sets applicant indian tribe member status to false" do
    ...
    context 'Updating an existing family' do
      ...
      it 'adds person records for new family members' do
        expect(family.family_members.map(&:person).count).to eq 1
      end
