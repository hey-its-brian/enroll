  ...
  context 'when qhp feature is enabled' do
    ...
    let(:esi_mec_evidence) do
      FactoryBot.create(:esi_mec_evidence, eligibility: aptc_csr_eligibility, _type: 'FinancialAssistance::Evidences::EsiMecEvidence', key: :esi_mec_evidence, title: 'Esi Mec Evidence', determined_at: TimeKeeper.date_of_record,
                                           description: 'ESI MEC Evidence Description', current_state: "pending")
    end

    let(:create_embed_docs) do
      build_eligibilities
      [esi_mec_evidence].each do |evidence|
    ...
    context 'FDSH Esi Mec eligible response' do
      include_context 'FDSH ESI MEC sample response'
      ...
      context 'when aasm_state is verified' do
        ...
        it 'should return attested status' do
          @applicant.reload
          esi_mec_evidence = @applicant.aptc_csr_eligibility.esi_mec_evidence
          expect(esi_mec_evidence.attested?).to be_truthy
          expect(esi_mec_evidence.verification_outstanding).to be_falsey
        end
      end
      ...
      context 'when aasm_state is outstanding' do
        ...
        context 'when not enrolled' do
          ...
          it 'should return negative_response_received' do
            @applicant.reload
            esi_mec_evidence = @applicant.aptc_csr_eligibility.esi_mec_evidence
            expect(esi_mec_evidence.negative_response_received?).to be_truthy
            expect(esi_mec_evidence.verification_outstanding).to be_falsey
          end
        end
        ...
        context 'when enrolled' do
          ...
          context 'with aptc used' do
            ...
            it 'returns outstanding' do
              ...
              @applicant.reload
              esi_mec_evidence = @applicant.aptc_csr_eligibility.esi_mec_evidence
              expect(esi_mec_evidence.current_state).to eq :outstanding
              expect(esi_mec_evidence.verification_outstanding).to be_truthy
              expect(esi_mec_evidence.is_satisfied).to be_falsey
            end
          end
          ...
          context 'without aptc used' do
            it 'returns outstanding' do
              ...
              @applicant.reload
              esi_mec_evidence = @applicant.aptc_csr_eligibility.esi_mec_evidence
              expect(esi_mec_evidence.current_state).to eq :negative_response_received
              expect(esi_mec_evidence.verification_outstanding).to be_falsey
              expect(esi_mec_evidence.due_on).not_to be_present
            end
          end
          ...
          context 'with csr used' do
            it 'returns outstanding' do
              ...
              expect(esi_mec_evidence.current_state).to eq :outstanding
              expect(esi_mec_evidence.verification_outstanding).to be_truthy
              expect(esi_mec_evidence.due_on).to be_present
            end
          end
        end
      end
