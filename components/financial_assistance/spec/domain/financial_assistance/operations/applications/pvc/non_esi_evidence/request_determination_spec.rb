...
RSpec.describe FinancialAssistance::Operations::Applications::Pvc::NonEsiEvidence::RequestDetermination, dbclean: :after_each do
  ...
  describe 'helper methods' do
    ...
    describe '#pvc_logger' do
      it 'creates a logger with the correct file path' do
        logger = subject.send(:pvc_logger)
        expect(logger).to be_a(Logger)
      end

      it 'memoizes the logger instance' do
        logger1 = subject.send(:pvc_logger)
        logger2 = subject.send(:pvc_logger)
  ...
  describe 'integration scenarios' do
    context 'when feature flag is disabled' do
      ...
      it 'completes the full workflow successfully' do
        ...
        expect(result).to be_success
        expect(result.value!).to include('Successfully published the pvc payload for application with hbx_id')
    ...
    context 'when feature flag is enabled with mixed valid/invalid applicants' do
      ...
      it 'handles mixed scenarios correctly' do
        ...
        if result.success?
          ...
          expect(result).to be_failure
          expect(result.failure).to include('PVC process failed') # Error details may vary
        end
      end
    end
