...
module FinancialAssistance
  module Operations
    module Evidences
      module EsiMec
        # This operation automatically extends the due date for ESI evidences for specific families. Families are selected based on a criteria.
        class CallHub
          ...
          def build_and_validate_payload_entity(application, evidence)
            ...
            if entity_monad.failure?
              failure_reason = "Application validity: Esi Mec Evidence verification request failed due to #{entity_monad.failure}"
          ...
          def validate_applicant(application_entity, evidence, application)
            ...
            validation_result = check_eligibility_rules(applicant_entity, :esi_mec)


            return Success(true) if validation_result.success?
            errors = validation_result.failure
            failure_reason = "Applicant validity: Esi Mec Evidence verification request failed due to #{errors}"
          ...
          def build_event(payload)
            ...
            event('events.fdsh.evidences.esi_determination_requested', attributes: payload, headers: headers.merge!({ esi_mec_payload_format: "json" }))
          end
        end
      end
    end
